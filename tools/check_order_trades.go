...
func main() {
	var (
		...
		symbol    = flag.String("symbol", "", "交易对符号")
		orderID   = flag.Int64("order-id", 0, "订单ID")
		useTestnet = flag.Bool("testnet", false, "使用测试网")
	)
	flag.Parse()

	if *apiKey == "" || *secretKey == "" || *symbol == "" || *orderID == 0 {
		log.Fatal("❌ 错误: 必须提供 api-key, secret-key, symbol 和 order-id")
	}
	...
	trades, err := getOrderTrades(context.Background(), baseURL, *apiKey, *secretKey, *symbol, *orderID)
	if err != nil {
		log.Fatalf("❌ 获取成交记录失败: %v", err)
	}

	if len(trades) == 0 {
		log.Println("⚠️  未找到成交记录")
		return
	}

	log.Printf("✓ 找到 %d 条成交记录:", len(trades))
	for _, trade := range trades {
		log.Printf("  交易ID: %d, 价格: %.4f, 数量: %.4f, 时间: %s",
			trade.TradeID,
			trade.Price,
			trade.Quantity,
...
func getOrderTrades(ctx context.Context, baseURL, apiKey, secretKey, symbol string, orderID int64) ([]TradeDetail, error) {
